...
export default function LoginSimplePage() {
  ...
  const startLogin = async () => {
    try {
      ...
      const initiateUrl = new URL(`${idpBaseUrl}/api/idp/initiate`)

      if (blocksKey) {
        initiateUrl.searchParams.set('x-blocks-key', blocksKey)
      }
      initiateUrl.searchParams.set('clientId', clientId)
      initiateUrl.searchParams.set('redirectUri', redirectUri)
      ...
      if (data.redirect_uri) {
        const authorizeUrl = new URL(
          data.redirect_uri,
          `${idpBaseUrl}/api/Authentication/Authorize`,
        )
