...
export function EditRouteModal({ keyDetails, onClose }: EditRouteProps) {
  const { isPending, mutateAsync } = useUilmSaveLanguageKey();
  const projectKey = useUilmProjectKey();

  const form = useForm<FormData>({
    defaultValues: {
      routes: keyDetails.routes?.length ? keyDetails.routes : [""],
    },
  ...
  const deleteRoute = async (index: number) => {
    ...
    try {
      ...
      if (res?.success) {
        showSuccessToast({ description: "Route deleted successfully" });
      } else {
        showErrorToast({ errors: res?.errorMessage || "Delete failed" });
      }
    } catch (error) {
      showErrorToast({
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await mutateAsync(payload);
      if (res?.success) {
        showSuccessToast({ description: "Routes updated successfully" });
        onClose();
      } else {
