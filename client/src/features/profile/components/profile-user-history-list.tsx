...
import type { IHistories } from "@/features/profile/model/profile-user.types";
...
export function ProfileUserHistoryList({ isLoading, data }: HistoryListProps) {
  const columns: ColumnDef<IHistories>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex w-[200px] items-center">
            <span>{formatDistanceToNow(new Date(row.original.LastUpdatedDate), { addSuffix: true })}</span>
          </div>
        ),
      },
      {
              ...
              <div className="flex flex-col">
                <span>
                  {deviceInfo.Device
                    ...
                    : "Unknown Device"}{" "}
                  {deviceInfo.Model ? `${deviceInfo.Model.charAt(0).toUpperCase() + deviceInfo.Model.slice(1)}` : ""}
                </span>
                <span>
