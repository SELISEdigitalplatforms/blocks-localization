...
export class ModuleDetailsPage {
  ...
  async openGlossaryTab() {
    ...
    await expect(this.page.getByRole("heading", { name: /Tagged Glossaries/ })).toBeVisible();
  }
  ...
  async expectGlossaryTagged(glossaryName: string) {
    await expect(this.page.getByRole("cell", { name: glossaryName, exact: true })).toBeVisible();
  }

  async expectNoGlossariesTagged() {
    await expect(this.page.getByText("No glossaries tagged to this module")).toBeVisible();
  }
}
